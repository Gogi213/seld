...
class TelegramBot {
  ...
  formatSignal(symbolData, timeframe) {
    ...
    const signal = symbolData[`percentileSignal_${timeframe}`];
    
    if (!signal) {
      return null;
    }

    const priceChange = symbolData.priceChange_1m || 0;
    const volumeK = Math.round(symbolData.volume_1m / 1000);
    const percentile = Math.round(symbolData[`volumePercentile_${timeframe}`] || 0);
    
    const changeIcon = priceChange > 0 ? '📈' : '📉';
    const changeStr = priceChange > 0 ? `+${priceChange.toFixed(1)}` : priceChange.toFixed(1);
    
    return `${changeIcon} ${symbol} ${timeframe} ${changeStr}% ${volumeK}K ${percentile}%`;
  }
  ...
  async sendSignalsSummary(signalsData) {
    ...
    // Собираем активные сигналы
    for (const [symbol, symbolData] of Object.entries(signalsData)) {
      if (symbolData.percentileSignal_1m) {
        const volumeK = Math.round(symbolData.volume_1m / 1000);
      ...
      if (symbolData.percentileSignal_5m) {
        const volumeK = Math.round(symbolData.volume_1m / 1000);
        const change = symbolData.priceChange_1m || 0;
        const changeStr = change > 0 ? `+${change.toFixed(1)}` : change.toFixed(1);
        const icon = change > 0 ? '📈' : '📉';
